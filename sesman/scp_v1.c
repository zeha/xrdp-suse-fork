...
scp_v1_process(struct SCP_CONNECTION* c, struct SCP_SESSION* s)
{
  ...
  if (scount == 0)
  {
    ...
    if (SCP_SESSION_TYPE_XVNC == s->type)
    {
      ...
      display = session_start(s->width, s->height, s->bpp, s->layout, s->username,
                              s->password, s->wm, data, SESMAN_SESSION_TYPE_XVNC);
    }
    else if (SCP_SESSION_TYPE_XRDP == s->type)
    {
      log_message(&(g_cfg->log), LOG_LEVEL_INFO, "starting X11rdp session...");
      display = session_start(s->width, s->height, s->bpp, s->layout, s->username,
                              s->password, s->wm, data, SESMAN_SESSION_TYPE_XRDP);
    }
    else
    {
      log_message(&(g_cfg->log), LOG_LEVEL_INFO, "starting Xdmx session...");
      display = session_start(s->width, s->height, s->bpp, s->layout, s->username,
                              s->password, s->wm, data, SESMAN_SESSION_TYPE_XDMX);
    }
